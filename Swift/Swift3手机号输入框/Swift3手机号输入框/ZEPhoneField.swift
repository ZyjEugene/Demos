...
//  Swift3手机号输入框
//
//  Created by tongqu on 2017/7/12.
//  Copyright © 2017年 Eugene Space. All rights reserved.
//

import UIKit

class ZEPhoneField: UITextField {
    ...
    //当本视图的父类视图改变的时候 继承自UIView的子类添加等操作的事件通知方法
    override func willMove(toSuperview newSuperview: UIView?) {
        //监听值改变通知事件
        if newSuperview != nil {
            NotificationCenter.default.addObserver(self, selector: #selector(phoneNumberFormat(_:)), name: Notification.Name.UITextFieldTextDidChange, object: nil)
        } else {
...
extension String
{
    ...
    {
        get{
            //self.index(self.startIndex, offsetBy: index) 根据字符的起始位置和偏移位，获取元素的索引
